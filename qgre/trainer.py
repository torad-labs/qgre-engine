...
class QGRETrainer:
    ...
    def _get_prompt_tier(self, metadata: dict) -> str:
        from qgre.curriculum import get_prompt_tier

        return get_prompt_tier(metadata, self._difficulty_column)

    def _record_mastery_and_advance(
        self, reward_results, active_qualities, batch, metrics, batch_contexts
    ):
        from qgre.curriculum import record_mastery_and_advance
        ...
        record_mastery_and_advance(
            ...
            tier_advance_phase=self._tier_advance_phase,
            tier_advance_threshold=self._tier_advance_threshold,
            global_step=self.global_step,
            metrics=metrics,
        )

    def _apply_difficulty_gate(self):
        from qgre.curriculum import apply_difficulty_gate

        apply_difficulty_gate(self.game_state, self._dataloader, self._difficulty_column)
